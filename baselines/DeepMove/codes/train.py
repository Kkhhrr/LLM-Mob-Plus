...
def get_metrics(target, scores, num_classes=None):
    """
    计算Acc@1, Acc@5, Acc@10, NDCG@10, weighted F1
    target: torch cuda Variable
    scores: torch cuda Variable (logits or log-probs)
    num_classes: int, 类别数
    """
    import numpy as np
    ...
    target = target.data.cpu().numpy()
    val, idxx = scores.data.topk(10, 1)
    predx = idxx.cpu().numpy()  # shape: (batch, 10)
    ...
    acc10 = acc10 / batch_size
    ndcg10 = ndcg10 / batch_size
    # weighted F1
    if num_classes is None:
        num_classes = np.max(target) + 1
    weighted_f1 = f1_score(y_true, y_pred, average='weighted', labels=np.arange(num_classes))
    return acc1, acc5, acc10, ndcg10, weighted_f1


def run_simple(data, run_idx, mode, lr, clip, model, optimizer, criterion, mode2=None):
    """mode=train: return model, avg_loss
       mode=test: return avg_loss, metrics_dict, users_rnn_acc"""
    run_queue = None
    if mode == 'train':
        model.train(True)
    ...
    for c in range(queue_len):
        ...
        if mode == 'train':
            ...
            # 只统计Top-1
            acc = (scores.data.topk(1, 1)[1].cpu().numpy().flatten() == target.data.cpu().numpy().flatten()).sum()
            users_acc[u][1] += acc
            all_targets.append(target.data.cpu().numpy())
            all_scores.append(scores.data.cpu().numpy())
    ...
    if mode == 'train':
        ...
        acc1, acc5, acc10, ndcg10, weighted_f1 = get_metrics(all_targets_tensor, all_scores_tensor, num_classes=all_scores.shape[1])
        metrics_dict = {
            'avg_loss': avg_loss,
            'avg_acc': avg_acc,
            'acc1': acc1,
            ...
            'users_rnn_acc': users_rnn_acc
        }
        return avg_loss, metrics_dict, users_rnn_acc
