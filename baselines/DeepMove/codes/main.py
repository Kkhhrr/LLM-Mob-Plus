...
def run(args):
    ...
    metrics = {'train_loss': [], 'valid_loss': [], 'accuracy': [], 'valid_acc': {},
               'acc1': [], 'acc5': [], 'acc10': [], 'ndcg10': [], 'weighted_f1': []}
    ...
    for epoch in range(parameters.epoch):
        ...
        avg_loss, metrics_dict, users_acc = run_simple(data_test, test_idx, 'test', lr, parameters.clip, model,
                                                      optimizer, criterion, parameters.model_mode)
        print('==>Test Acc@1:{:.4f} Acc@5:{:.4f} Acc@10:{:.4f} NDCG@10:{:.4f} wF1:{:.4f} Loss:{:.4f}'.format(
            metrics_dict['acc1'], metrics_dict['acc5'], metrics_dict['acc10'], metrics_dict['ndcg10'], metrics_dict['weighted_f1'], avg_loss))

        metrics['valid_loss'].append(avg_loss)
        metrics['accuracy'].append(metrics_dict['avg_acc'])
        metrics['valid_acc'][epoch] = users_acc
        metrics['acc1'].append(metrics_dict['acc1'])
        metrics['acc5'].append(metrics_dict['acc5'])
        metrics['acc10'].append(metrics_dict['acc10'])
        metrics['ndcg10'].append(metrics_dict['ndcg10'])
        metrics['weighted_f1'].append(metrics_dict['weighted_f1'])

        save_name_tmp = 'ep_' + str(epoch) + '.m'
        torch.save(model.state_dict(), SAVE_PATH + tmp_path + save_name_tmp)

        scheduler.step(metrics_dict['avg_acc'])
        lr_last = lr
    ...
    metrics_view = {'train_loss': [], 'valid_loss': [], 'accuracy': [], 'acc1': [], 'acc5': [], 'acc10': [], 'ndcg10': [], 'weighted_f1': []}
