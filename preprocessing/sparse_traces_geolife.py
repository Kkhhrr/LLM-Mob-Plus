...
def process_geolife_for_deepmove(epsilon=50, num_samples=2, hour_gap=12, session_min_len=5, user_min_sessions=10):
    ...
    if Path(sp_time_file).is_file():
        ...
        print(f"Loading pre-generated locations from {locations_file}...")

        locs_df = pd.read_csv(locations_file, index_col=0)

        locs_df.index.name = 'location_id'
        
        locs_df['center'] = locs_df['center'].apply(wkt.loads)
        locs = gpd.GeoDataFrame(locs_df, geometry='center', crs="EPSG:4326")
        
    else:
        print("No cache file found. Processing from raw data. This will take a long time...")
        ...
        print("Generating locations via clustering. This is the slowest step...")
        ...
        print(f"Saving intermediate staypoints file to {sp_time_file}...")
    ...
    for user_id, user_df in tqdm(sp_time.groupby('user_id'), desc="Building Sessions"):
    ...
    print(f"Step 2 finished. Found {len(sessions)} valid users after session filtering.")
    ...
    for old_uid, user_sessions in tqdm(sessions.items(), desc="Finalizing Structure"):
    ...
    print(f"Step 3 finished. Data successfully saved to {output_path}")
